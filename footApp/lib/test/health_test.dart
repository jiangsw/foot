...
class _HealthKitDemoState extends State<HealthKitDemo> {
  ...
  void revoke() async {
    ...
    try {
      ...
      log('revoke', _consentTextController, LogOptions.error,
          error: e.message!);
    }
  }
  ...
  showSampleSet(SampleSet sampleSet) {
    for (var samplePoint in sampleSet.samplePoints) {
      for (var field in samplePoint.getDataType.fields) {
        Map m = samplePoint.fieldValues;
        logger.d(m['steps(i)']);

        logger.d("Field: " +
            field.name +
            ' Value: ' +
            samplePoint.fieldValues.toString());
      }
    }
  }
